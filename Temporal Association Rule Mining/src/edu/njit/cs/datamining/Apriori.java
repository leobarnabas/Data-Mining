...
public class Apriori {
	 ...
	 * 
	 * @param fileName
	 *            filename specified by the user that contains transactions
	 */
	public void runAlgorithm(String fileName) {
		readTransactions(fileName);
		String op_fileName = fileName + "_" + min_support + "-support_"
				+ min_confidence + "-confidence.txt";
		try {
		...
		for (String key : oneItemset.keySet()) {
			double support = ((double) oneItemset.get(key) / transactions
					.size());
		...
		for (int i = 0; i < uniqueItems.size(); i++) {
			for (int j = i + 1; j < uniqueItems.size(); j++) {
				...
				double support = (double) getItemsetCount(transactions, itemset)
						/ transactions.size();
				if (support >= min_support) {
		...
		while (true) {
			...
			for (int i = 0; i < supportItemList.size() - 1; i++) {
				...
				for (int j = i + 1; j < supportItemList.size(); j++) {
					...
					if (supportItemList.get(i).substring(0, idx1)
							.equals(supportItemList.get(j).substring(0, idx2))) {
						String newItem = supportItemList.get(i)
								+ supportItemList.get(j).substring(idx2);
						double support = (double) getItemsetCount(transactions,
								newItem) / transactions.size();
						if (support >= min_support) {
		...
		printItem("\nAssociation rules meeting minimum confidence of "
				+ min_confidence);
		if (ruleSet.size() == 0)
			printItem("None");
		for (String rule : ruleSet)
			printItem(rule);
		if (writer != null) {
			System.out.println("\nPlease check the output file " + op_fileName
					+ " for the steps");
			writer.close();
		}
	 ...
	 * @param str
	 */
	public void Permutation(List<List<String>> result, List<String> prefix,
			List<String> str) {
		int n = str.size();
		if (n == 0) {
			result.add(prefix);
	...
	public int getItemsetCount(List<String> transactions, String itemset) {
		...
		for (String transaction : transactions) {
			...
			for (int i = 0; i < items.length; i++) {
				...
				if (!isContains(transaction, items[i].trim())) {
					allOccurring = false;
					break;
				}
			}
	...
	public boolean isContains(String source, String subterm) {
		String pattern = "\\b" + subterm + "\\b";
		Pattern pttrn = Pattern.compile(pattern);
		Matcher match = pttrn.matcher(source);
		return match.find();
	}
}
